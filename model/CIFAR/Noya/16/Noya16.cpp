            ...
            for(int m=0;m<50;m++){
                ...
                for(int i=0;i<7;i++){
                    for(int j=0;j<7;j++){
                        ...
                        int y=j*2;
                        int num=m*49+i*7+j;
                        int idx=num%CipherNUM2;
                        int idy=num/CipherNUM2;
                        for(int k=0;k<25;k++){
        ...
        for(int b=0;b<BATCH_SIZE;b++){
              for(int n=0;n<10;n++){
                for(int i=0;i<BLOCKS;i++){
                    for(int j=0;j<CipherNUM2;j++){
                        if(i*CipherNUM2+j<50*7*7)
                            weight[n][b*256*BLOCKS+i*256+j]=params.FCWeights[n*50*7*7+i*CipherNUM2+j];
                    }
                }
            }
        }
         ...
         for(int i=0;i<BATCH_SIZE;i++){
            ...
            for(int k=0;k<10;k++){
                ...
                for(int j=0;j<2450;j++){
                    tmp+=res[k][i*256*BLOCKS+(j/CipherNUM2)*256+j%CipherNUM2];
                }
                val.push_back(tmp);
            }
