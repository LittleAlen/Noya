...
vector<SealBfvCiphertext> Noya(const Params &params, const vector<SealBfvCiphertext> &inputE, const SealBfvEnvironment &env)
{
    ...
    // Conv2 Layer
    {
        ...
        vector<vector<vector<double>>> weight(77,vector<vector<double>>(25,vector<double>(env.poly_modulus_degree,0.0)));
        vector<SealBfvPlaintext>WeightsP(77*25,SealBfvPlaintext());
        vector<vector<double>> biases(77,vector<double>(env.poly_modulus_degree,0));

        vector<SealBfvPlaintext> BiasesP(77,SealBfvPlaintext());
                    ...
                    for(int k=0;k<25;k++){
                        for(int ii=0;ii<4;ii++){
                            for(int jj=0;jj<4;jj++){
                                weight[idx][k][idy*256+(x+ii)*16+y+jj]=params.conv2Weights[m*25*4*4+k*4*4+ii*4+jj];
                            }
                        }
                    }
                }
        ...
        for(int i=0;i<50;i++){
            for(int j=0;j<7*7;j++){
                ...
                int idx=num%77;
                int idy=num/77;
                biases[idx][idy*256]=params.conv2Biases[i];
            }
        }
        for(int i=0;i<77;i++){
        ...
        for(int i=0;i<77;i++){
            ...
            rotate_inplace(resultE[i],-i,env);
        }
        SealBfvCiphertext tmp;
        add_many(resultE,tmp,env);
        rotate_add(tmp,{-128},tmp,env);
        intermediateResultsE.clear();
        vector<SealBfvCiphertext>().swap(intermediateResultsE);
    ...
    // FC Layer
    {
        ...
        vector<SealBfvPlaintext> WeightsP;
        vector<SealBfvPlaintext> BiasesP;
        vector<vector<double>>weight(5,vector<double>(env.poly_modulus_degree,0));
        vector<double> vMsgs(env.poly_modulus_degree,0.0);
        ...
        for(int n=0;n<10;n+=2){
            for(int i=0;i<32;i++){
                for(int j=0;j<77;j++){
                    if(i*77+j>=50*7*7)
                        continue;
                    weight[n/2][i*256+j]=params.FCWeights[n*50*7*7+i*77+j];
                    weight[n/2][i*256+128+j]=params.FCWeights[(n+1)*50*7*7+i*77+j];
                }
            }
        }
        ...
        for(int i=0;i<5;i++){
            vMsgs[0]=params.FCBiases[2*i];
            vMsgs[128]=params.FCBiases[2*i+1];
    ...
    {   
        ...
        for(int i=0;i<BATCH_SIZE;i++){
            ...
            for(int j=0;j<5;j++){
                ...
                for(int r=0;r<32;r++){
                    for(int k=0;k<77;k++)
                        node1+=res[j][r*256+k],node2+=res[j][r*256+128+k];
                }
                val.push_back(node1);
                val.push_back(node2);
                // cout<<node<<" ";
