...
vector<SealBfvCiphertext> Noya(const Params &params, const vector<SealBfvCiphertext> &inputE, const SealBfvEnvironment &env)
{
    ...
    // FC1 Layer
    {
        ...
        vector<SealBfvCiphertext> resultE;
        vector<vector<double>> weight(25*5,vector<double>(8192,0));
        vector<SealBfvPlaintext> WeightsP;
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int i=0;i<100;i++){
                vMsgs[b*256*BLOCKS+(i/25)*256+(i%25)]=params.FC1Biases[i];
            }
        }
       ...
       // rotate_add(tmp,{-25},tmp,env);
        rotate_add(tmp,{-25,-25*2,-25*4},tmp,env);
    ...
    // FC2 Layer
    {
        ...
        vector<SealBfvPlaintext> WeightsP;
        vector<vector<double>> weight(2, vector<double>(env.poly_modulus_degree,0.0));
        vector<SealBfvPlaintext> BiasesP;
        vector<double> vMsgs(env.poly_modulus_degree,0.0);
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int k=0;k<10;k++)
            for(int j=0;j<100;j++){
                weight[k/8][b*256*BLOCKS+(j/25)*256+(k%8)*25+j%25]=params.FC2Weights[k*100+j];
            }
        }
        for(int i=0;i<2;i++)
       ...
       for(int k=0;k<2;k++){
        for(int b=0;b<BATCH_SIZE;b++){
            for(int j=0;j<8;j++){
                if(k*8+j<params.FC2Biases.size())
                    vMsgs[b*256*BLOCKS+j*25]=params.FC2Biases[k*8+j];
        }
        }
    ...
    for (size_t batchIndex = 0; batchIndex < data.size(); batchIndex++)
    {
        ...
        for(int i=0;i<BATCH_SIZE;i++){
            ...
            for(int k=0;k<10;k++){
                double tmp=0;
                for(int j=0;j<100;j++){
                    tmp+=res[k/8][i*256*BLOCKS+(j/25)*256+(k%8)*25+j%25];
                }
                val.push_back(tmp);
            }
