...
vector<SealBfvCiphertext> Noya(const Params &params, const vector<SealBfvCiphertext> &inputE, const SealBfvEnvironment &env)
{
    ...
    // FC1 Layer
    {
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int i=0;i<100;i++){
                vMsgs[b*256*BLOCKS+(i/7)*256+(i%7)]=params.FC1Biases[i];
            }
        }
       ...
       // SealBfvCiphertext tmp1();
        ...
        rotate_add(tmp,{-7,-7*2,-7*4,-7*8},tmp,env);
    ...
    // FC2 Layer
    {
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int k=0;k<10;k++)
            for(int j=0;j<100;j++){
                weight[0][b*256*BLOCKS+(j/7)*256+k*7+j%7]=params.FC2Weights[k*100+j];
            }
        }
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int j=0;j<10;j++){
                    vMsgs[b*256*BLOCKS+j*7]=params.FC2Biases[j];
        }
        }
    ...
    for (size_t batchIndex = 0; batchIndex < data.size(); batchIndex++)
    {
        ...
        for(int i=0;i<BATCH_SIZE;i++){
            ...
            for(int k=0;k<10;k++){
                double tmp=0;
                for(int j=0;j<100;j++){
                    tmp+=res[0][i*256*BLOCKS+(j/7)*256+k*7+j%7];
                }
                val.push_back(tmp);
            }
