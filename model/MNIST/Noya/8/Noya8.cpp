...
vector<SealBfvCiphertext> Noya(const Params &params, const vector<SealBfvCiphertext> &inputE, const SealBfvEnvironment &env)
{
    ...
    // FC1 Layer
    {
        ...
        vector<SealBfvCiphertext> resultE;
        vector<vector<double>> weight(13*5,vector<double>(8192,0));
        vector<SealBfvPlaintext> WeightsP;
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int i=0;i<100;i++){
                vMsgs[b*256*BLOCKS+(i/13)*256+(i%13)]=params.FC1Biases[i];
            }
        }
       ...
       // SealBfvCiphertext tmp1();
       
        rotate_add(tmp,{-13,-13*2,-13*4,-13*8},tmp,env);
    ...
    // FC2 Layer
    {
        ...
        vector<SealBfvPlaintext> WeightsP;
        vector<vector<double>> weight(1, vector<double>(env.poly_modulus_degree,0.0));
        vector<SealBfvPlaintext> BiasesP;
        vector<double> vMsgs(env.poly_modulus_degree,0.0);
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int k=0;k<10;k++)
            for(int j=0;j<100;j++){
                weight[0][b*256*BLOCKS+(j/13)*256+k*13+j%13]=params.FC2Weights[k*100+j];
            }
        }
        
        WeightsP.emplace_back(move(SealBfvPlaintext(weight[0],env,FC2_SCALE)));
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            for(int j=0;j<10;j++){
                    vMsgs[b*256*BLOCKS+j*13]=params.FC2Biases[j];
        }
        }
    ...
    for (size_t batchIndex = 0; batchIndex < data.size(); batchIndex++)
    {
        ...
        for(int i=0;i<BATCH_SIZE;i++){
            ...
            for(int k=0;k<10;k++){
                double tmp=0;
                for(int j=0;j<100;j++){
                    tmp+=res[0][i*256*BLOCKS+(j/13)*256+k*13+j%13];
                }
                val.push_back(tmp);
            }
