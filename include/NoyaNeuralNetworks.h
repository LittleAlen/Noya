...
namespace mycryptonets
{
     void fc(vector<SealBfvCiphertext const *> inputPtr,
            ...
            size_t dotLen,//the size of left matrix's columns/right matrix's rows
...
#pragma omp parallel for                
    ...
    vector<vector<double>> ConvertToConvolutionRepresentation(vector<vector<vector<double>>> &data) {
                ...
                for (int j = 0; j < 13; j++) {
                    ...
                    for (int ii = 0; ii < 5; ii++) {
                        for (int jj = 0; jj < 5; jj++) {
                            int xx = x + ii;
                            int yy = y + jj;
                            msgs[ii * 5 + jj].push_back(data[k][xx][yy]);
                        }
                    }
                }
            }
            //padding 256-169=87 
            for(int pad=0;pad<87;pad++){
                for (int ii = 0; ii < 5; ii++) {
                    for (int jj = 0; jj < 5; jj++) {
                        msgs[ii * 5 + jj].push_back(0);
                    }
                }
            }
        }
    ...
    vector<vector<double>> ConvertToConvolutionRepresentationNew(vector<vector<vector<vector<double>>>> &data) {
        vector<vector<double>> msgs(3*4*4);
        // output is 16x16
        for(int k=0;k<data.size();k++){
            for (int i = 0; i < 16; i++) {
                for (int j = 0; j < 16; j++) {
                    ...
                    for(int d=0;d<3;d++)
                        for (int ii = 0; ii < 4; ii++) {
                            for (int jj = 0; jj < 4; jj++) {
                                int xx = x + ii;
                                int yy = y + jj;
                                msgs[d*4*4+ii * 4 + jj].push_back(data[k][d][xx][yy]);
                            }
                        }
                }
            }
